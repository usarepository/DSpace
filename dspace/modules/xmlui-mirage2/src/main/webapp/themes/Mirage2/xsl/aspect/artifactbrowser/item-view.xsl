...
<xsl:stylesheet
   ...
   <xsl:template match="mets:fileGrp[@USE='LICENSE']">
    ...
    <xsl:template name="impact-plumx">
        <div id="impact-plumx" style="clear:right" class="table">
            ...
            <a>
                ...
                <xsl:variable name="plumx_data-border" select="confman:getProperty('plumx.data-border')"/>
                <xsl:if test="confman:getProperty('plumx.data-border')">
                    <xsl:attribute name="data-border"><xsl:value-of select="$plumx_data-border"/></xsl:attribute>
                </xsl:if>
                &#xFEFF;
            </a>
    ...
    <xsl:variable name='identifier_doi'
                  select='//dri:meta/dri:pageMeta/dri:metadata[@element="citation_doi" and not(@qualifier)]'/>
    <xsl:variable name='identifier_handle'
                  select="substring-after(//dri:meta/dri:pageMeta/dri:metadata[@element='request' and @qualifier='URI'],'handle/')"/>
</xsl:stylesheet>
